...
$import_stats = $action_scheduler->getTaskStats('wpwps_import_order');
$sync_stats = $action_scheduler->getTaskStats('wpwps_sync_order');
...
$last_sync = get_option('wpwps_last_orders_sync', '');
...
<div class="wrap wpwps-wrap">
    <h1 class="wp-heading-inline"><?php echo esc_html__('Order Sync', 'wp-woocommerce-printify-sync'); ?></h1>
    ...
    <?php else : ?>
            ...
            <div class="wpwps-col-12">
                <div class="wpwps-card">
                    <div class="wpwps-card-header">
                        <h2><?php echo esc_html__('Order Sync Status', 'wp-woocommerce-printify-sync'); ?></h2>
                    </div>
                    <div class="wpwps-card-body">
                        ...
                        <div class="wpwps-sync-actions">
                            <button class="button button-primary" id="wpwps-import-orders" data-nonce="<?php echo esc_attr(wp_create_nonce('wpwps-admin-ajax-nonce')); ?>">
                                <i class="dashicons dashicons-download"></i> <?php echo esc_html__('Import New Orders', 'wp-woocommerce-printify-sync'); ?>
                            </button>
                            
                            <a href="<?php echo esc_url(admin_url('admin.php?page=wpwps-settings#order-sync')); ?>" class="button button-secondary">
                                <i class="dashicons dashicons-admin-generic"></i> <?php echo esc_html__('Sync Settings', 'wp-woocommerce-printify-sync'); ?>
                            </a>
                        </div>
                    </div>
                </div>
        ...
        <div class="wpwps-row">
            ...
            <div class="wpwps-col-6">
                <div class="wpwps-card">
                    <div class="wpwps-card-header">
                        <h2><?php echo esc_html__('Recently Synced Orders', 'wp-woocommerce-printify-sync'); ?></h2>
                    </div>
                    <div class="wpwps-card-body">
                        <?php if (empty($recent_orders)) : ?>
                            <p class="wpwps-no-data">
                                <?php echo esc_html__('No recently synced orders.', 'wp-woocommerce-printify-sync'); ?>
                            </p>
                        <?php else : ?>
                            <table class="wpwps-table">
                                <thead>
                                    <tr>
                                        <th><?php echo esc_html__('Order', 'wp-woocommerce-printify-sync'); ?></th>
                                ...
                                <tbody>
                                    <?php foreach ($recent_orders as $order) : ?>
                                        <tr>
                                            <td>
                                                <a href="<?php echo esc_url($order['edit_url']); ?>" target="_blank">
                                                    <?php echo esc_html($order['order_number']); ?>
                                                </a>
                                                <div class="wpwps-order-meta">
                                                    <?php echo esc_html($order['customer_name']); ?>
                                                </div>
                                            </td>
                                            <td>
                                                <span class="wpwps-status wpwps-status-<?php echo esc_attr($order['status']); ?>">
                                                    <?php echo esc_html($order['status_label']); ?>
                                                </span>
                                            </td>
                                            <td>
                                                <?php echo esc_html($order['last_synced']); ?>
                                            </td>
                                            <td>
                                                <div class="wpwps-action-buttons">
                                                    <a href="<?php echo esc_url($order['edit_url']); ?>" class="button button-small" title="<?php echo esc_attr__('Edit', 'wp-woocommerce-printify-sync'); ?>">
                                                        <span class="dashicons dashicons-edit"></span>
                                                    </a>
                                                    <?php if (!empty($order['printify_url'])) : ?>
                                                        <a href="<?php echo esc_url($order['printify_url']); ?>" class="button button-small" title="<?php echo esc_attr__('View in Printify', 'wp-woocommerce-printify-sync'); ?>" target="_blank">
                                                            <span class="dashicons dashicons-external"></span>
                                                        </a>
                                                    <?php endif; ?>
                                                    <button class="button button-small wpwps-sync-order" 
                                                            data-order-id="<?php echo esc_attr($order['id']); ?>" 
                                                            data-printify-id="<?php echo esc_attr($order['printify_id']); ?>"
                            ...
                            <div class="wpwps-view-all">
                                <a href="<?php echo esc_url(admin_url('edit.php?post_type=shop_order&wpwps_filter=printify')); ?>" class="button">
                                    <?php echo esc_html__('View All Printify Orders', 'wp-woocommerce-printify-sync'); ?>
                                </a>
                            </div>
                        <?php endif; ?>
                    </div>
        ...
        <div class="wpwps-row">
            <div class="wpwps-col-12">
                <div class="wpwps-card">
                    <div class="wpwps-card-header">
                        <h2><?php echo esc_html__('Printify Orders', 'wp-woocommerce-printify-sync'); ?></h2>
                    </div>
                    <div class="wpwps-card-body">
                        <div class="wpwps-table-container">
                            <div class="wpwps-table-tools">
                                <div class="wpwps-search">
                                    <input type="text" id="wpwps-order-search" placeholder="<?php echo esc_attr__('Search orders...', 'wp-woocommerce-printify-sync'); ?>" />
                                </div>
                            ...
                            <table class="wpwps-table wpwps-orders-table" id="wpwps-orders-table">
                                <thead>
                                    <tr>
                                        <th><?php echo esc_html__('Order #', 'wp-woocommerce-printify-sync'); ?></th>
                                        <th><?php echo esc_html__('Date', 'wp-woocommerce-printify-sync'); ?></th>
                                        <th><?php echo esc_html__('Customer', 'wp-woocommerce-printify-sync'); ?></th>
                                        <th><?php echo esc_html__('Status', 'wp-woocommerce-printify-sync'); ?></th>
                                        <th><?php echo esc_html__('Total', 'wp-woocommerce-printify-sync'); ?></th>
                                        <th><?php echo esc_html__('Printify Status', 'wp-woocommerce-printify-sync'); ?></th>
                                        <th><?php echo esc_html__('Actions', 'wp-woocommerce-printify-sync'); ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr class="wpwps-loading-row">
                                        <td colspan="7" class="wpwps-loading">
                                            <span class="spinner is-active"></span> <?php echo esc_html__('Loading orders...', 'wp-woocommerce-printify-sync'); ?>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
<script type="text/template" id="wpwps-order-row-template">
    <tr data-order-id="{{ id }}">
        <td>
            <a href="{{ edit_url }}" target="_blank">#{{ order_number }}</a>
            <div class="wpwps-order-meta">
                <span>{{ printify_id }}</span>
            </div>
        </td>
        <td>{{ date }}</td>
        <td>{{ customer_name }}</td>
        <td>
            <span class="wpwps-status wpwps-status-{{ status }}">{{ status_label }}</span>
        </td>
        <td>{{ formatted_total }}</td>
        <td>
            <span class="wpwps-status wpwps-status-printify-{{ printify_status_slug }}">{{ printify_status }}</span>
        </td>
        <td>
            <div class="wpwps-action-buttons">
                ...
                <# if (printify_url) { #>
                    <a href="{{ printify_url }}" class="button button-small" title="<?php echo esc_attr__('View in Printify', 'wp-woocommerce-printify-sync'); ?>" target="_blank">
                        <span class="dashicons dashicons-external"></span>
                    </a>
                <# } #>
                <button class="button button-small wpwps-sync-order" 
...
<script type="text/template" id="wpwps-no-orders-template">
    <tr class="wpwps-no-orders-row">
        <td colspan="7" class="wpwps-no-orders">
            <?php echo esc_html__('No Printify orders found.', 'wp-woocommerce-printify-sync'); ?>
        </td>
    </tr>
</script>