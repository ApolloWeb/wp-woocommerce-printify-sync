...
async function getFileDescription(file) {
  ...
  try {
    ...
    // Remove "Authored by" or any unnecessary metadata
    description = description.replace(/(Authored by:.*|Generated by:.*)/gi, "").trim();

    // Enforce 50-character max length (truncate at word boundary)
    if (description.length > 50) {
      description = description.substring(0, 47).trim(); // Ensure we don't cut a word in half
